...
import com.wishes.assistant.myapplication.activity.LibraryLoginPageActivity;
...
public class Library extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (Utils.isLoginLibSystem(getActivity())) {
            //如果已登陆，则显示正文
                    ...
                    public void onMenuItemClick(View view, int position) {
                        switch (position) {
                            case 0:
                                ...
                                break;
                            case 1:
                                if (!SharePreferenceUtils.isLoginLibSystem(getActivity().getApplicationContext())) {
                                    //如果未登录，则跳转到登录界面
                                    Intent intent = new Intent(getActivity(), LibraryLoginPageActivity.class);
                                    startActivity(intent);
                                } else {
                                    SharePreferenceUtils.clearData(SharePreferenceUtils.ACCOUNT_TYPE.LIBRARY, getActivity().getApplicationContext()
                                    );//清空登录信息
                                    Toasty.info(getActivity().getApplicationContext(), "已注销，请重新登录!", Toast.LENGTH_SHORT, true).show();
                                }
                                break;
                        }
                    }
