...
import com.wishes.assistant.customs.dialog.GradeSelectorDialog;
...
public class Grade extends SkinCompatActivity implements AAH_FabulousFragment.Callbacks {
    //Android cline
    private ClingManager mClingManager;
    private static String START_TUTORIAL_KEY = "GradeActivityShowedGuide";

    public static final String mContentType = "score";
    ...
    private GradeSelectorDialog dialogFrag;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0:
                    ...
                    ReLoginDialog dialog = new ReLoginDialog(Grade.this, new ReLoginDialog.DialogCallBackListener() {
                        @Override
                ...
                case 1:
                    ...
                    //得到图片的base64编码
                    //final String base64Image = "data:image/jpg;base64," + Base64Img.getImgStr(bitmap);//在线预览时加上字符头
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mLayout.post(new Runnable() {
            ...
            public void run() {
                ...
                ValueAnimator colorAnim = ObjectAnimator.ofInt(mTitleBar, "backgroundColor", getPrimaryColor(), Color.argb(255, 98, 133, 199));
                ...
                animatorSet.start();
                //改变状态栏颜色，做成沉浸式状态栏
                getWindow().setStatusBarColor(Color.argb(255, 98, 133, 199));
            }
        ...
        //从数据库中取出相应值
        SQLiteHelper helper = SQLiteHelper.getInstance(getApplicationContext());
        refreshTime = helper.getRefreshTime("REFRESHTIMETABLE", "1");


        dialogFrag = GradeSelectorDialog.newInstance();
        ...
        img.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    //构造来源表明
                    String fromTable = "Grade";

                    //將刷新时间存入数据库
                    SQLiteHelper helper = SQLiteHelper.getInstance(getApplicationContext());
                    helper.insertImgToDb(fromTable, bitName, base64Img);

                    Intent intent = new Intent(Grade.this, SimpleSampleActivity.class);
    ...
    protected void onResume() {
        if (UserGuide.isFirstStart(Grade.this, START_TUTORIAL_KEY)) {
            //当是第一次启动Activity时
            ...
            mClingManager.addCling(new Cling.Builder(this)
                    .setTitle("考试成绩查询")
                    .setContent("这是用于查询入校以来考试成绩的界面。点击右下角菜单按钮，选择学年、学期以及格式即可查询。(当Cookie过期时你只需要输入验证码即可重新登录。)")
                    .build());

            mClingManager.addCling(new Cling.Builder(this)
                    .setTitle("Tips")
                    .setContent("点击这里可以展开菜单，在选择学年、学期和格式后，你会看到你的成绩图片。(当Cookie过期时你只需要输入验证码即可重新登录。)")
                    .setMessageBackground(getResources().getColor(R.color.btn_grade))
                    .setTarget(new ViewTarget(this, R.id.fab))
                    .build());
            mClingManager.addCling(new Cling.Builder(this)
                    .setTitle("Tips")
                    .setContent("在这里你会看到你的成绩的图片。点击图片可以查看详情或保存哟~")
                    .setMessageBackground(getResources().getColor(R.color.btn_grade))
            ...
            mClingManager.setCallbacks(new ClingManager.Callbacks() {
                ...
                public void onClingHide(int position) {
                    ...
                    if (position == 3) {
                        ...
                        mClingManager = null;
                        UserGuide.changeActivityStartTimes(Grade.this, START_TUTORIAL_KEY);
                    }
                }
            });
    ...
    private void doGet() {
        ...
        if (cancle) {
            ...
            mGetDataTask = new getData(mXn, mXq, mType);
    ...
    public class getData extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            if (getImgData(Constants.GradePicUrl, xq, xn, sj)) {
                return true;
            }
            return false;
        }
        ...
        private boolean getImgData(String url, String xq, String xn, String sj) {
            ...
            try {
                String imgUrl = Constants.GradePicUrl + "xn=" + xn + "&xq=" + xq + "&sj=" + sj;
                Log.d("Grade imgURL:", imgUrl);
